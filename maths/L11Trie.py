...
class Trie:
    ...
    def contains_recruit(self, node, word, index):
        ...
        :return:
        """
        # 递归到底的情况，注意最后一个元素的 is_word 是不是 True
        if index == len(word):
            if node.is_word:
                return True
            return False
    ...
    def add_recruit(self, node, word, index):
        ...
        dst_element = word[index]
        # 如果当前节点的 next 的 dict 键中不包含 dst_element
        if node.next.get(dst_element, Node) is None:
            # 就为这个键新建一个 Node
...
if __name__ == "__main__":
    ...
    print("初始化是否为空：%s" % test.is_empty())
    print("此时的 size ：%s" % test.get_size())
    ...
    print("此时的 test 的 size：%s" % test.get_size())
    print('"hel" 是否是 test 的前缀：%s' % test.is_prefix('hel'))
